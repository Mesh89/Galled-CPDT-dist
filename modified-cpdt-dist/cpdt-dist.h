...
int nonred_colors;
ull good_triplets = 0, good_fans = 0;
ull tot_good_triplets = 0, tot_good_fans = 0;
...
void color_snnode_blue(cdp_node_t* ui) {
	...
	for (size_t i = 0; i < ui->Cnum; i++) {
		...
		// Lemma 1.5
		good_fans += Ca * comb2(u->Cstar-uCi + reds_down);
		good_fans -= Ca * (u->C2-comb2(uCi) + comb2(reds_down));
		good_fans -= Ca * (u->SSstar + reds2_down - uSSi);
		good_fans += Ca * (reds2_down + u->C2pstar-uC2pi);
		good_fans += Ca * ((u->Cstar-uCi)*(u->tot_reds-ui->tot_reds-reds_down));
	}
}
...
void color_cpnode_blue(cdp_node_t* vj) {
	...
	for (size_t i = 0; i < vj->Cnum; i++) {
		...
		// Lemma 1.4
		good_fans += Ca * (v->Cstar-vCi+reds_down) * (vi->Cstar-viCi+redsj_down);
		good_fans -= Ca * (vi->CC-(viCi*vCi)+(reds_down*redsj_down));
		good_fans += Ca * (v->Cstar-vCi) * (vi->tot_reds-vj->tot_reds-redsj_down);
	}
}
...
void leaves_coloring(tree_node* v, bool decolor) {
	...
	tot_good_triplets += good_triplets;
	good_triplets = 0;
	tot_good_fans += good_fans;
	good_fans = 0;
...
std::pair<ull,ull> triplet_distance(tree* t1, tree* t2) {
	...
	return std::make_pair(tot_good_triplets, tot_good_fans);
}
